...
import com.example.finsplore.dto.GoalRequest;
...
public class GoalController {
    ...
    // receive the goal request and update the user budget
    @PatchMapping("/set")
    ...
    // get the user goal
    @GetMapping("/{userId}")
    public ResponseEntity<GoalRequest> getBudget(@PathVariable Long userId) {
        return userService.getUserGoal(userId)
                .map(budget -> ResponseEntity.ok(new GoalRequest(userId, budget)))
